    ...
    def buildGemmsConfig(self):
        ...
        if self.context["isLinear"]:
            ...
            if self.context["useFlux"]:
                ...
                if self.context["predictorRecompute"]:
                    self.context["matmulConfigs"]["lduh_y"] =     MatmulConfig(nVarPad, nDof, nDof, nVarPad*nDof, nDofPad, nVarPad*nDof , 1, 1, 0, 0,  "lduh_y",  "nopf", "gemm")
                    if self.context["nDim"]>=3:
                        self.context["matmulConfigs"]["lduh_z"] = MatmulConfig(nVarPad, nDof, nDof, nVarPad*nDof2, nDofPad, nVarPad*nDof2, 1, 1, 0, 0,  "lduh_z",  "nopf", "gemm")
                else:
            ...
            if self.context["useNCP"] or self.context['useViscousFlux']:
                if self.context["predictorRecompute"]:
                    self.context["matmulConfigs"]["gradQ_x"] =     MatmulConfig(nVarPad, nDof, nDof, nVarPad , nDofPad, nVarPad      , 1, 1, 0, 1, "gradQ_x", "nopf", "gemm")
                    self.context["matmulConfigs"]["gradQ_y"] =     MatmulConfig(nVarPad, nDof, nDof, nVarPad*nDof, nDofPad, nVarPad*nDof , 1, 1, 0, 1, "gradQ_y", "nopf", "gemm")
                    if self.context["nDim"]>=3:
                        self.context["matmulConfigs"]["gradQ_z"] = MatmulConfig(nVarPad, nDof, nDof, nVarPad*nDof2, nDofPad, nVarPad*nDof2, 1, 1, 0, 1, "gradQ_z", "nopf", "gemm")
                else:
