...
detected_errors_pattern=re.compile("( [0-9]*\.[0-9]*).*JobTableStatistics::printStatistics.*team ([0-9]).*soft errors injected = ([0-9]*).*detected soft errors = ([0-9]*).*limited tasks = ([0-9]*).*healed tasks = ([0-9]*) ")


def prefixArrayToDeltaArray(prefixArray):
  previous = 0
  result = []
  for t in range(len(prefixArray)):
    current=prefixArray[t]
...
def parseFile(filename, teams):
  ...
  for line in file:
    ...
    if m:
     print (line)
     print ("appending",m.group(3), float(m.group(3)), "to team",int(m.group(2)) )
     dict[int(m.group(2))]["timestamps"].append(float(m.group(3)))
    ...
    if m:
     ...
     dict[int(m.group(2))]["healed_tasks_accum"].append(int(m.group(6)))
  ...
  for i in range(teams):
    ...
    dict[i]["detected_errors_delta"] = prefixArrayToDeltaArray(dict[i]["detected_errors_accum"])
    ...
    dict[i]["injected_errors_delta"] = prefixArrayToDeltaArray(dict[i]["injected_errors_accum"])
    ...
    dict[i]["limited_tasks_delta"] = prefixArrayToDeltaArray(dict[i]["limited_tasks_accum"])
    
    dict[i]["healed_tasks_delta"] = prefixArrayToDeltaArray(dict[i]["healed_tasks_accum"])
  return dict
...
def plotHealedTasks(ax, x, data, label, marker, colour):
  handle=ax.plot(x, data, label=label, marker=marker, color=colour, linestyle="--")
  return handle
...
if __name__=="__main__":
  ...
  print(result[0]['timestamps'])
  print(result[1]['timestamps'])

  handle_t0 = plotTimeStepSizesDelta(ax,range(0, len(result[0]['timestamps'])), result[0]['timestamps'], "with injected error - timestamps team 0", "x", "r")
  handle_t1 = plotTimeStepSizesDelta(ax,range(0, len(result[1]['timestamps'])), result[1]['timestamps'], "with injected error - timestamps team 1", "s", "b")
  ...
  ax.axvspan(0, error_idx, facecolor='green', alpha=0.5)
  ax.axvspan(error_idx, limiter_idx, facecolor='orange', alpha=0.5)
  ax.axvspan(limiter_idx, 100, facecolor='green', alpha=0.5)
  ...
  #handle_healed_t1 = plotHealedTasks(ax2, range(0, len(result[1]['healed_tasks_delta'])), result[1]['healed_tasks_delta'], "with injected error - corrected tasks on team 1", "s", "b")

  # added these three lines
  #lns = handle_limited
  lns = handle_t0 + handle_t1 # + handle_healed_t0 + handle_healed_t1
  ...
  plt.tight_layout()
  plt.savefig("timestamps_large_error.pdf", bbox_inches='tight', dpi=300)
  plt.show()
