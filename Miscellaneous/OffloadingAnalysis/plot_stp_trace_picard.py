...
x_label="Ranks"
y_label="Total Picard Iterations in Space Time Predictor per Rank"
...
if __name__=="__main__":
  ...
  plot_stp_trace_single(ax1, x, iterations_own, time_own, x_label, y_label, 'g', 'Migratable Job (local)', iterations_pred)

  (x, iterations_remote, time_remote) = pt.parseDir(dirname, ranks, threads, timestep, basename3)
  plot_stp_trace_single(ax1, x, iterations_remote, time_remote, x_label, y_label, 'r', 'Migratable Job (remote)', [iterations_pred[i]+iterations_own[i] for i in range(0,ranks-1)])
  ...
  plt.savefig(dirname+"stp_its_"+str(timestep)+".pdf")
  import tikzplotlib
  tikzplotlib.save(dirname+"stp_its_"+str(timestep)+".tex")
