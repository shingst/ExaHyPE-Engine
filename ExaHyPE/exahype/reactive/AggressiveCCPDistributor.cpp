...
void exahype::reactive::AggressiveCCPDistributor::enable() {
  _isEnabled = true;
}

void exahype::reactive::AggressiveCCPDistributor::disable() {
  _isEnabled = false;
}

void exahype::reactive::AggressiveCCPDistributor::computeIdealLoadDistribution(int enclaveCells, int skeletonCells) {
...
exahype::reactive::AggressiveCCPDistributor& exahype::reactive::AggressiveCCPDistributor::getInstance() {
  static AggressiveCCPDistributor aggressiveDist;
  return aggressiveDist;
}

void exahype::reactive::AggressiveCCPDistributor::printOffloadingStatistics() {
  int nnodes = tarch::parallel::Node::getInstance().getNumberOfNodes();
  int myRank = tarch::parallel::Node::getInstance().getRank();
...
void exahype::reactive::AggressiveCCPDistributor::resetRemainingTasksToOffload() {
...
void exahype::reactive::AggressiveCCPDistributor::handleEmergencyOnRank(int rank) {
  _emergenciesPerRank[rank]++;
  logInfo("handleEmergencyOnRank()","emergencies for rank:"<<_emergenciesPerRank[rank]);
}

void exahype::reactive::AggressiveCCPDistributor::updateLoadDistribution() {
  ...
  if(myRank == criticalRank && !isVictim) {
     for(int i=0; i<nnodes; i++) {
       if(_idealTasksToOffload[i]>0) {
         ...
         if(!exahype::reactive::ReactiveContext::getInstance().isBlacklisted(i)) {
...
bool exahype::reactive::AggressiveCCPDistributor::selectVictimRank(int& victim) {
