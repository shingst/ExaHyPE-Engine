...
//    sends++;
    // logInfo("submitOrSendStealablePredictionJob","element "<<job->_element<<" predictor time stamp"<<job->_predictorTimeStamp<<" predictor time step size "<<job->_predictorTimeStepSize);
     ...
     int tag = exahype::stealing::StealingManager::getInstance().getStealingTag();
...
bool exahype::solvers::ADERDGSolver::tryToReceiveTaskBack(exahype::solvers::ADERDGSolver* solver, const void* cellDescription) {
  ...
  return exahype::stealing::StealingManager::getInstance().progressReceiveBackRequests();
...
bool exahype::solvers::ADERDGSolver::ReceiveBackJob::run( bool isCalledOnMaster ) {
  ...
  while(run) {
    ...
    run = receivedTaskBack || exahype::stealing::StealingManager::getInstance().hasOutstandingRequestOfType(exahype::stealing::RequestType::receiveBack);
  }
...
//  VT_begin(event_stealingManager);
  ...
  switch (_state) {
    ...
    case State::Resume:
      //logInfo("stealingManager", " resumed ");
      _state = State::Running;
      break;
    case State::Paused:
      //logInfo("stealingManager", " paused ");
      result = false;
      break;
...
void exahype::solvers::ADERDGSolver::resumeStealingManager() {
  //logInfo("resumeStealingManager", "resuming ");
