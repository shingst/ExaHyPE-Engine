...
void exahype::solvers::ADERDGSolver::cleanUpStaleTaskOutcomes(bool isFinal) {
  ...
  while( (i< unsafe_size || isFinal) && gotOne) {
    ...
    if(!gotOne) break;
    logInfo("cleanUpStaleTaskOutcomes()", " time stamp of key ="<<key.timestamp);
    i++;
...
void exahype::solvers::ADERDGSolver::sendTaskOutcomeToOtherTeams(MigratablePredictionJob *job) {
    ...
    for(int i=0; i<teams; i++) {
      if(i!=interCommRank) {
        logDebug("sendReplicatedSTPToOtherTeams"," team "<< interCommRank
                                                         ...
                                                         <<" time stamp = "<<job->_predictorTimeStamp
                                                         <<" to team "<<i);
        sendMigratablePredictionJobOutcomeOffload(&lduh[0],
                                           &lQhbnd[0],
                                           &lFhbnd[0],
                                           &lGradQhbnd[0],
										   i,
    ...
    for(int i=0; i<teams; i++) {
      if(i!=interCommRank) {
          logDebug("sendReplicatedSTPToOtherTeams"," team "<< interCommRank
                                                   ...
                                                   <<" time stamp = "<<job->_predictorTimeStamp
                                                   <<" to team "<<i);
          isendMigratablePredictionJobOutcome(&(data->_lduh[0]),
                                      &(data->_lQhbnd[0]),
                                      &(data->_lFhbnd[0]),
									  &(data->_lGradQhbnd[0]),
                                      i,
    ...
    exahype::offloading::OffloadingManager::getInstance().submitRequests(sendRequests, (teams-1)*(NUM_REQUESTS_MIGRATABLE_COMM+1), tag, -1,
...
void exahype::solvers::ADERDGSolver::receiveMigratableJob(int tag, int src, exahype::solvers::ADERDGSolver *solver) {
  MPI_Request receiveRequests[(NUM_REQUESTS_MIGRATABLE_COMM)];
  ...
  int canComplete = 0;
  int ierr = MPI_Testall(NUM_REQUESTS_MIGRATABLE_COMM, &receiveRequests[0], &canComplete, MPI_STATUSES_IGNORE);
...
void exahype::solvers::ADERDGSolver::receiveBackMigratableJob(int tag, int src, exahype::solvers::ADERDGSolver *solver) {
  ...
  solver->irecvMigratablePredictionJobOutcome(
      &(data->_lduh[0]),
      &(data->_lQhbnd[0]),
      &(data->_lFhbnd[0]),
	  &(data->_lGradQhbnd[0]),
  ...
  solver->irecvMigratablePredictionJobOutcome(
	  ...
	  recvRequests);
  exahype::offloading::OffloadingManager::getInstance().submitRequests(
      recvRequests, NUM_REQUESTS_MIGRATABLE_COMM_SEND_OUTCOME, tag, src,
      exahype::solvers::ADERDGSolver::MigratablePredictionJob::receiveBackHandler,
  ...
  while( (receivedTask || receivedTaskBack) && iprobesCounter<MaxIprobesInOffloadingProgress && !terminateImmediately ) {
    ...
    if(receivedReplicaKey) {
      ...
      exahype::offloading::OffloadingManager::getInstance().submitRequests(
           ...
           solver,
           false);
       exahype::offloading::JobTableStatistics::getInstance().notifyReceivedKey();
    }
    ...
    if(receivedReplicaAck) {
       ...
       if(buffer==REQUEST_JOB_ACK) {
         ...
         solver->isendMigratablePredictionJobOutcome(
                                             &data->_lduh[0],
                                             &data->_lQhbnd[0],
                                             &data->_lFhbnd[0],
         ...
         exahype::offloading::OffloadingManager::getInstance().submitRequests(
                             sendRequests, NUM_REQUESTS_MIGRATABLE_COMM_SEND_OUTCOME, statRepAck.MPI_TAG, statRepAck.MPI_SOURCE,
                             MigratablePredictionJob::sendHandlerTaskSharing,
                             exahype::offloading::RequestType::sendReplica,
                             solver, false);
         delete[] sendRequests;
...
bool exahype::solvers::ADERDGSolver::ReceiveBackJob::run( bool isCalledOnMaster ) {
  ...
  while(run) {
    ...
    if(receivedTaskBack) {
      ...
      MPI_Request recvRequests[NUM_REQUESTS_MIGRATABLE_COMM_SEND_OUTCOME];
      _solver.irecvMigratablePredictionJobOutcome(
        lduh, lQhbnd,
        lFhbnd, lGradQhbnd, statMapped.MPI_SOURCE, statMapped.MPI_TAG, commMapped, recvRequests);
...
void exahype::solvers::ADERDGSolver::isendMigratablePredictionJobOutcome(
  double *lduh,
  double *lQhbnd,
  double *lFhbnd,
  double *lGradQhbnd,
  ...
  timing += MPI_Wtime();
  if(timing > OFFLOADING_SLOW_OPERATION_THRESHOLD)
    logError("isendMigratablePredictionJobOutcome()", " took "<<timing<<"s");
#endif
};
...
void exahype::solvers::ADERDGSolver::irecvMigratablePredictionJobOutcome(
    double *lduh,
    double *lQhbnd,
    double *lFhbnd,
	double *lGradQhbnd,
  ...
  timing += MPI_Wtime();
  if(timing > OFFLOADING_SLOW_OPERATION_THRESHOLD)
    logError("irecvMigratablePredictionJobOutcome()", " took "<<timing<<"s");
#endif
};

void exahype::solvers::ADERDGSolver::recvMigratablePredictionJobOutcome(
  double *lduh,
  double *lQhbnd,
  double *lFhbnd,
  double *lGradQhbnd,
  ...
  timing += MPI_Wtime();
  if(timing > OFFLOADING_SLOW_OPERATION_THRESHOLD)
    logError("recvMigratablePredictionJobOutcome()", " took "<<timing<<"s");
#endif

};

#if defined(UseMPIOffloading)
void exahype::solvers::ADERDGSolver::recvMigratablePredictionJobOutcomeOffload(
    double *lduh,
    double *lQhbnd,
    double *lFhbnd,
	double *lGradQhbnd,
  ...
  timing += MPI_Wtime();
  if(timing > OFFLOADING_SLOW_OPERATION_THRESHOLD)
    logError("recvMigratablePredictionJobOutcomeOffload()", " took "<<timing<<"s");
#endif
};

void exahype::solvers::ADERDGSolver::sendMigratablePredictionJobOutcomeOffload(
  double *lduh,
  double *lQhbnd,
  double *lFhbnd,
  double *lGradQhbnd,
  ...
  timing += MPI_Wtime();
  if(timing > OFFLOADING_SLOW_OPERATION_THRESHOLD)
    logError("sendMigratablePredictionJobOutcomeOffload()", " took "<<timing<<"s");
#endif
};
#endif
