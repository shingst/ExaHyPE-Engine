...
//  }
  ...
  std::fill(&_currentBlacklistSnapshot[0], &_currentBlacklistSnapshot[nnodes], 0);
  if(completed_fused) {
    ...
    for(int i=0; i<nnodes; i++) {
       ...
       for(int j=0; j<nnodes; j++) {
         ...
         _currentBlacklistSnapshot[j] += _currentFusedDataReceiveBuffer[offsetBlacklistValues+j];
       }
    }
    _fusedGatherRequest = MPI_REQUEST_NULL;
  }
