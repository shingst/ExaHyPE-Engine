...
class exahype::mappings::GlobalRollback {
 ...
 if ( fineGridVertex.isAdjacentToDomainOf( tarch::parallel::Node::getInstance().getRank() ) ) {
   ...
     ...
     *
     * This operation is called for every cell that is either moved or is 
     * adjacent to a cell on the same level which is transported. Please note
     * that the operation is called copy and not move, i.e. no data is to be 
     * destroyed here. If data has to be destroyed, Peano invokes a delete
     * call later on.
     *
     ...
     *                            again. Please note that even without any manual 
     *                            data update, receivedCoarseGridVertices and
     *                            workersCoarseGridVertices have to differ. On a worker,
     *                            the latter for example always is (logically) not-inside and remote,
     *                            while it might be inside on the master.
