...
def parseRanksNodesCoreCounts(jobs):
    """
    Returns a list of tuples of the form (ranks,nodes,coreCounts, serversPerRank).
    coreCounts is an ordered dict with key 'cores' and value 'background job'
    consumer tasks. Servers per rank specifies how many progression server processes should be run on 
    e.g., a smartNIC per application rank.
    """
    RanksNodesCoreCounts = collections.namedtuple("ranksNodesCoreCounts", \
        "nodes ranks coreCounts serverRanks text")
    CoreCount = collections.namedtuple("coreCount", \
        "cores consumers")
    # example: "29 x 29 x {4:2,8:4} x 1 ,\\n758 x 29 x {1:1} x 1"
    entryPattern      = re.compile(r"(\s*([0-9]+)\s*x\s*([0-9]+)\s*x\s*\{(([0-9]|\:|,|\s)+)\}*x\s*([0-9]+))");
    # example: 4:2,8:4
    # example: 4:2
    coreCountsPattern = re.compile(r"([0-9]+)\s*\:\s*([0-9]+)");
    
    ranksNodesCoreCountsList = []
    for entryMatch in re.findall(entryPattern, jobs["ranks_nodes_cores_servers"]):
        coreCountsList = []
        ...
        if int(nodes) > int(ranks):
            ...
            sys.exit()
        ranksNodesCoreCountsList.append( RanksNodesCoreCounts( ranks=ranks, nodes=nodes, coreCounts=coreCountsList, serverRanks=serversPerRank , text=entryMatch[0] ) )
        
    if not ranksNodesCoreCountsList:
        print("ERROR: could not find any 'ranks_nodes_cores_servers' entries in the form: 'ranks_nodes_cores = <ranks> x <nodes> x {<cores0>:<consumertasks0>,<cores1>:<consumertasks1>,...} x serversPerRank'.\n"\
              "       Valid examples: 'ranks_nodes_cores = 29 x 29 x {1:1} x 1', 'ranks_nodes_cores = 758 x 29 x {8:4,16:8} x 1'",file=sys.stderr)
        sys.exit()
...
def parseOptionsFile(optionsFile,ignoreMetadata=False):    
    ...
    ranksNodesCoreCountsServers = parseRanksNodesCoreCounts(jobs)
    # runs
    runNumbers       =["+"]
    ...
    options = Options(
      ...
      jobClass             = jobClass,\
      islands              = islands,\
      ranksNodesCoreCountsServers = ranksNodesCoreCountsServers,\
