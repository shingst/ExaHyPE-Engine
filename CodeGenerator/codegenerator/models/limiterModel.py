...
class LimiterModel(AbstractModelBaseClass):
    ...
    def buildGemmsConfig(self):
        ...
        self.context["matmulConfigs"]["uh2lob_x"] =           MatmulConfig(nData   , nDof, nDof, nData         , nDofPad, nDataPad      , 1, 1, 0, 1, "uh2lob_x", "nopf", "gemm") # input slice not aligned
        if(self.context["nDim"]>=3):
            self.context["matmulConfigs"]["uh2lob_y"] =       MatmulConfig(nDataPad, nDof, nDof, nDof*nDataPad , nDofPad, nDof*nDataPad , 1, 1, 1, 1, "uh2lob_y", "nopf", "gemm") #M is padded in both input and output
