...
#if defined(_CURVILINEAR)
    CurvilinearTransformation curvilinear_transformation;
#endif    
    
  public:
    ElasticWaveEquation(double maximumMeshSize,int maximumAdaptiveMeshDepth,int DMPObservables,exahype::solvers::Solver::TimeStepping timeStepping,std::vector<std::string>& cmdlineargs);
    ...
    **/
    ...
    void riemannSolver(double* _FL,double* _FR,const double* const _QL,const double* const _QR,const double dt,const int normalNonZeroIndex, bool isBoundaryFace, int faceIndex) override;
    ...
    void extractDampingParameters(const double* const Q, double& d_x, double& d_y, double& d_z);
    void extractMaterialParameters(const double* const Q, double& rho, double& cp, double &cs);
    void extractMetricParameters(const double* const Q, double& jacobian,
				 double& q_x, double&q_y, double& q_z,
				 double& r_x, double&r_y, double& r_z,
				 double& s_x, double&s_y, double& s_z);
    void extractVariables(const double* const Q, double& u,  double& v,  double& w,
			  double& sigma_xx,double& sigma_yy,double& sigma_zz,
			  double& sigma_xy,double& sigma_xz,double& sigma_yz);

    void extractAuxiliaryParameters(const double* const Q,double* Px,double* Py,double* Pz);
