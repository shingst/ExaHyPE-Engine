...
! 27.05.2011 
!
RECURSIVE SUBROUTINE LinSolve(N,A,b,x)
  IMPLICIT NONE
  INTEGER       :: N
  REAL          :: A(N,N), b(N), x(N)  
  !
...
RECURSIVE  SUBROUTINE LinSolve2(N,M,A,b,x,iErr)
   IMPLICIT NONE
   INTEGER       :: N,M
   INTEGER       :: iErr 
...
RECURSIVE SUBROUTINE MatrixInverse(N,A,iA)
  IMPLICIT NONE
  INTEGER       :: N
  REAL          :: A(N,N), iA(N,N)
  !
...
!
RECURSIVE SUBROUTINE CL2(M,nEqn,nDOF,nConst,A,C,R,iErr) 
   !USE operators_mod 
   IMPLICIT NONE 
   ! Argument list 
