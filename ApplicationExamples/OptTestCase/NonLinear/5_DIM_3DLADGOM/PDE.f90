...
RECURSIVE SUBROUTINE PDEEigenvectors(R,L,iR,Q,nv) 
  ...
  iR = 0. 
        ...
        iRM(1,1)   = irho/cp**2/2
        iRM(1,7)   = 1/cp/2
        iRM(1,13)  = -1/cp**2/rho*(cp*rho*uv(1)*Q(13) -s11)/2*ialpha
        iRM(2,4)   = irho/cs**2/2
        iRM(2,8)   = 1/cs/2
        iRM(2,13)  = -1/cs**2/rho*(uv(2)*cs*rho*Q(13) -s12)/2*ialpha
        iRM(3,6)   = irho/cs**2/2
        iRM(3,9)   = 1/cs/2
        iRM(3,13)  = -1/cs**2/rho*(uv(3)*cs*rho*Q(13) -s13)/2*ialpha
        ...
        iRM(11,9)  = -1/cs/2
        iRM(11,13) = 1/cs**2/rho*(uv(3)*cs*rho*Q(13) +s13)/2*ialpha
        iRM(12,4)  = irho/cs**2/2
        iRM(12,8)  = -1/cs/2
        iRM(12,13) = 1/cs**2/rho*(uv(2)*cs*rho*Q(13) +s12)/2*ialpha
        iRM(13,1)  = irho/cp**2/2
        iRM(13,7)  = -1/cp/2
        iRM(13,13) = 1/cp**2/rho*(cp*rho*uv(1)*Q(13) +s11)/2*ialpha
        iRM(14,14) = 1 
        !
        R = MATMUL(   TM,  RM ) 
        iR = MATMUL( iRM, iTM ) 
