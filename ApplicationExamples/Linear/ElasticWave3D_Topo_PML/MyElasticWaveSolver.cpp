    ...
    for (int j=0; j< num_nodes; j++){
	  for (int i=0; i< num_nodes; i++){
	    ...
	    if( level <= getCoarsestMeshLevel()){ 
	      ...
	      //double xx  =  (offset_x+width_x*kernels::lobatto::nodes[basisSize-1][i]);
	      //double yy  =  (offset_y+width_y*kernels::lobatto::nodes[basisSize-1][j]);
	      //double zz  =  (offset_z+width_z*kernels::lobatto::nodes[basisSize-1][k]);
...
//	       if(x < 1.0) {
	    ...
	    double c =   6.0;
	    double d0x = (n+1)*c/(2*dpmlx)* log(1/tol)*0;  // remove *0 to enable PML setting
...
exahype::solvers::Solver::RefinementControl Elastic::MyElasticWaveSolver::refinementCriterion(const double* const luh,const tarch::la::Vector<DIMENSIONS,double>& center,const tarch::la::Vector<DIMENSIONS,double>& dx,double t,const int level) {
  ...
  bool elementOnSurface=left_vertex[0] < dx[1];
  bool elementbelowSurface=left_vertex[0] < 2*dx[1];
...
void Elastic::MyElasticWaveSolver::flux(const double* const Q,double** const F) {
  ...
  assertion8( !isnan(F[0][0]),"flux is nan at 0 0",jacobian,q_x,sigma_xx,q_y,sigma_xy,q_z,sigma_xz);
  assertion8( !isnan(F[0][1]),"flux is nan at 0 1",jacobian,q_x,sigma_xy,q_y,sigma_yy,q_z,sigma_yz);
  assertion8( !isnan(F[0][2]),"flux is nan at 0 2",jacobian,q_x,sigma_xz,q_y,sigma_yz,q_z,sigma_zz);
  assertion8( !isnan(F[0][9]),"flux is nan at 0 9",jacobian,q_x,sigma_xx,q_y,sigma_xy,q_z,sigma_xz);
  assertion8( !isnan(F[0][10]),"flux is nan at 0 10",jacobian,q_x,sigma_xy,q_y,sigma_yy,q_z,sigma_yz);
  assertion8( !isnan(F[0][11]),"flux is nan at 0 11",jacobian,q_x,sigma_xz,q_y,sigma_yz,q_z,sigma_zz);
  assertion8( !isnan(F[1][0]),"flux is nan at 1 0",jacobian,r_x,sigma_xx,r_y,sigma_xy,r_z,sigma_xz);
  assertion8( !isnan(F[1][1]),"flux is nan at 1 1",jacobian,r_x,sigma_xy,r_y,sigma_yy,r_z,sigma_yz);
  assertion8( !isnan(F[1][2]),"flux is nan at 1 2",jacobian,r_x,sigma_xz,r_y,sigma_yz,r_z,sigma_zz);
  assertion8( !isnan(F[1][18]),"flux is nan at 1 18",jacobian,r_x,sigma_xx,r_y,sigma_xy,r_z,sigma_xz);
  assertion8( !isnan(F[1][19]),"flux is nan at 1 19",jacobian,r_x,sigma_xy,r_y,sigma_yy,r_z,sigma_yz);
  assertion8( !isnan(F[1][20]),"flux is nan at 1 20",jacobian,r_x,sigma_xz,r_y,sigma_yz,r_z,sigma_zz);
  assertion8( !isnan(F[2][0]),"flux is nan at 2 0",jacobian,s_x,sigma_xx,s_y,sigma_xy,s_z,sigma_xz);
  assertion8( !isnan(F[2][1]),"flux is nan at 2 1",jacobian,s_x,sigma_xy,s_y,sigma_yy,s_z,sigma_yz);
  assertion8( !isnan(F[2][2]),"flux is nan at 2 2",jacobian,s_x,sigma_xz,s_y,sigma_yz,s_z,sigma_zz);
  assertion8( !isnan(F[2][27]),"flux is nan at 2 27",jacobian,s_x,sigma_xx,s_y,sigma_xy,s_z,sigma_xz);
  assertion8( !isnan(F[2][28]),"flux is nan at 2 28",jacobian,s_x,sigma_xy,s_y,sigma_yy,s_z,sigma_yz);
  assertion8( !isnan(F[2][29]),"flux is nan at 2 29",jacobian,s_x,sigma_xz,s_y,sigma_yz,s_z,sigma_zz);

  assertion8( std::isfinite(F[0][0]),"flux is not finite at 0 0",jacobian,q_x,sigma_xx,q_y,sigma_xy,q_z,sigma_xz);
  assertion8( std::isfinite(F[0][1]),"flux is not finite at 0 1",jacobian,q_x,sigma_xy,q_y,sigma_yy,q_z,sigma_yz);
  assertion8( std::isfinite(F[0][2]),"flux is not finite at 0 2",jacobian,q_x,sigma_xz,q_y,sigma_yz,q_z,sigma_zz);
  assertion8( std::isfinite(F[0][9]),"flux is not finite at 0 9",jacobian,q_x,sigma_xx,q_y,sigma_xy,q_z,sigma_xz);
  assertion8( std::isfinite(F[0][10]),"flux is not finite at 0 10",jacobian,q_x,sigma_xy,q_y,sigma_yy,q_z,sigma_yz);
  assertion8( std::isfinite(F[0][11]),"flux is not finite at 0 11",jacobian,q_x,sigma_xz,q_y,sigma_yz,q_z,sigma_zz);
  assertion8( std::isfinite(F[1][0]),"flux is not finite at 1 0",jacobian,r_x,sigma_xx,r_y,sigma_xy,r_z,sigma_xz);
  assertion8( std::isfinite(F[1][1]),"flux is not finite at 1 1",jacobian,r_x,sigma_xy,r_y,sigma_yy,r_z,sigma_yz);
  assertion8( std::isfinite(F[1][2]),"flux is not finite at 1 2",jacobian,r_x,sigma_xz,r_y,sigma_yz,r_z,sigma_zz);
  assertion8( std::isfinite(F[1][18]),"flux is not finite at 1 18",jacobian,r_x,sigma_xx,r_y,sigma_xy,r_z,sigma_xz);
  assertion8( std::isfinite(F[1][19]),"flux is not finite at 1 19",jacobian,r_x,sigma_xy,r_y,sigma_yy,r_z,sigma_yz);
  assertion8( std::isfinite(F[1][20]),"flux is not finite at 1 20",jacobian,r_x,sigma_xz,r_y,sigma_yz,r_z,sigma_zz);
  assertion8( std::isfinite(F[2][0]),"flux is not finite at 2 0",jacobian,s_x,sigma_xx,s_y,sigma_xy,s_z,sigma_xz);
  assertion8( std::isfinite(F[2][1]),"flux is not finite at 2 1",jacobian,s_x,sigma_xy,s_y,sigma_yy,s_z,sigma_yz);
  assertion8( std::isfinite(F[2][2]),"flux is not finite at 2 2",jacobian,s_x,sigma_xz,s_y,sigma_yz,s_z,sigma_zz);
  assertion8( std::isfinite(F[2][27]),"flux is not finite at 2 27",jacobian,s_x,sigma_xx,s_y,sigma_xy,s_z,sigma_xz);
  assertion8( std::isfinite(F[2][28]),"flux is not finite at 2 28",jacobian,s_x,sigma_xy,s_y,sigma_yy,s_z,sigma_yz);
  assertion8( std::isfinite(F[2][29]),"flux is not finite at 2 29",jacobian,s_x,sigma_xz,s_y,sigma_yz,s_z,sigma_zz);
}
...
void Elastic::MyElasticWaveSolver::riemannSolver(double* const FL_,double* const FR_,const double* const QL_,const double* const QR_,const double dt,const int normalNonZeroIndex, bool isBoundaryFace, int faceIndex){
  ...
  for (int i = 0; i < basisSize; i++) {
	...
	FR_l=0;
      ...
      assertion12( std::isfinite(FL[idx_FLR(i,j, 3)]), "FL  3 is not finite",i,j,norm_m_qr,mu_m,lam_m,n_m[0],n_m[1],n_m[2],FL_x,FL_y,FL_z);
      assertion12( std::isfinite(FL[idx_FLR(i,j, 4)]), "FL  4 is not finite",i,j,norm_m_qr,mu_m,lam_m,n_m[0],n_m[1],n_m[2],FL_x,FL_y,FL_z);
      assertion12( std::isfinite(FL[idx_FLR(i,j, 5)]), "FL  5 is not finite",i,j,norm_m_qr,mu_m,lam_m,n_m[0],n_m[1],n_m[2],FL_x,FL_y,FL_z);
      assertion12( std::isfinite(FR[idx_FLR(i,j, 3)]), "FR  3 is not finite",i,j,norm_p_qr,mu_p,lam_p,n_p[0],n_p[1],n_p[2],FR_x,FR_y,FR_z);
      assertion12( std::isfinite(FR[idx_FLR(i,j, 4)]), "FR  4 is not finite",i,j,norm_p_qr,mu_p,lam_p,n_p[0],n_p[1],n_p[2],FR_x,FR_y,FR_z);
      assertion12( std::isfinite(FR[idx_FLR(i,j, 5)]), "FR  5 is not finite",i,j,norm_p_qr,mu_p,lam_p,n_p[0],n_p[1],n_p[2],FR_x,FR_y,FR_z);
      assertion9( std::isfinite(FL[idx_FLR(i,j, 6)]), "FL  6 is not finite",i,j,norm_m_qr,mu_m,n_m[0],n_m[1],FL_x,FL_y);
      assertion9( std::isfinite(FL[idx_FLR(i,j, 7)]), "FL  7 is not finite",i,j,norm_m_qr,mu_m,n_m[0],n_m[2],FL_x,FL_z);
      assertion9( std::isfinite(FL[idx_FLR(i,j, 8)]), "FL  8 is not finite",i,j,norm_m_qr,mu_m,n_m[1],n_m[2],FL_y,FL_z);
      assertion9( std::isfinite(FR[idx_FLR(i,j, 6)]), "FR  6 is not finite",i,j,norm_p_qr,mu_p,n_p[0],n_p[1],FR_x,FR_y);
      assertion9( std::isfinite(FR[idx_FLR(i,j, 7)]), "FR  7 is not finite",i,j,norm_p_qr,mu_p,n_p[0],n_p[2],FR_x,FR_z);
      assertion9( std::isfinite(FR[idx_FLR(i,j, 8)]), "FR  8 is not finite",i,j,norm_p_qr,mu_p,n_p[1],n_p[2],FR_y,FR_z);
    }    
  }
