  ...
  if (tarch::la::equals(t,0.0)) {
	...
	for (int i=0; i< num_nodes; i++){
	  ...
	  if( level <= getCoarsestMeshLevel()){ 
	    ...
	    // q_x, q_y, q_z
	    luh[id_xyzf(k,j,i,numberOfVariables+4)]  = q_x[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+5)]  = q_y[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+6)]  = q_z[id_xyz(k,j,i)];
	    
	    // r_x, r_y, r_z
	    luh[id_xyzf(k,j,i,numberOfVariables+7)] = r_x[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+8)] = r_y[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+9)] = r_z[id_xyz(k,j,i)];
	    
	    // s_x, s_y, s_z
	    luh[id_xyzf(k,j,i,numberOfVariables+10)] = s_x[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+11)] = s_y[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+12)] = s_z[id_xyz(k,j,i)];
	    
	    // x,y,z
	    luh[id_xyzf(k,j,i,numberOfVariables+13)] = gl_vals_x[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+14)] = gl_vals_y[id_xyz(k,j,i)];
	    luh[id_xyzf(k,j,i,numberOfVariables+15)] = gl_vals_z[id_xyz(k,j,i)];
	  }
	  //	  std::cout << std::endl;
...
void Elastic::MyElasticWaveSolver::extractTransformation(const double* const Q,
  ...
  q_z     =Q[numberOfVariables + 6];
  r_x     =Q[numberOfVariables + 7];
  r_y     =Q[numberOfVariables + 8];
  r_z     =Q[numberOfVariables + 9];
  s_x     =Q[numberOfVariables + 10];
  s_y     =Q[numberOfVariables + 11];
...
void Elastic::MyElasticWaveSolver::prestress(double& T0_n, double& T0_m, double& T0_l, double* const x, double t, double* const l, double* const m, double* const n)
{
  ...
  boxcar(fz, x[2]-12.5, 1.5);
  //
  T0_l = T0_l+8.0   *fy*fz;

  boxcar(fy, x[1]-7.5,  1.5);
  boxcar(fz, x[2]-20.0, 1.5);
  //
  T0_l = T0_l+11.6   *fy*fz;

  boxcar(fy, x[1]-7.5,  1.5);
  boxcar(fz, x[2]-27.5, 1.5);
  //
  T0_l = T0_l-8.0   *fy*fz;
