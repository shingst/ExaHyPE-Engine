...
RECURSIVE SUBROUTINE PDEFlux(f,g,hz,Q)
  USE Parameters, ONLY : nVar, nDim, nPar
  USE iso_c_binding
  IMPLICIT NONE
  REAL :: f(nVar+nPar), g(nVar+nPar), h(nVar+nPar), hz(nVar+nPar), Q(nVar+nPar), V(nVar+nPar)
...
RECURSIVE SUBROUTINE PDENCP(BgradQ,Q,gradQ) 
   USE Parameters, ONLY :  nVar, nDim, epsilon1,nPar
   IMPLICIT NONE
   REAL, PARAMETER :: epsilon = 1e-14
  ...
  !
   ...
   CQz = 0. 
    ...
    !lam  = 3.685150e+01!Q(10)
    !mu   = 3.017425e+01!Q(11) 
    !irho = 1./2.7!1./Q(12)
...
! Original model
    ...
    AQx(7) = - irho*alpha * Qx(1) 		- Q(1)*irho*Qx(13)  
    AQx(8) = - irho*alpha * Qx(4) 		- Q(4)*irho*Qx(13)   
    AQx(9) = - irho*alpha * Qx(6) 		- Q(6)*irho*Qx(13)       
    !                                   
    ...
    BQy(6) =   0.0                      
    BQy(7) = - irho*alpha * Qy(4) 		- Q(4)*irho*Qy(13)  
    BQy(8) = - irho*alpha * Qy(2) 		- Q(2)*irho*Qy(13)   
    BQy(9) = - irho*alpha * Qy(5) 		- Q(5)*irho*Qy(13)      
    !                                   
    ...
    CQz(7) = - irho*alpha * Qz(6) 		- Q(6)*irho*Qz(13)  
    CQz(8) = - irho*alpha * Qz(5) 		- Q(5)*irho*Qz(13)  
    CQz(9) = - irho*alpha * Qz(3) 		- Q(3)*irho*Qz(13)    
    if( nDim .eq. 2) then
        BgradQ = AQx + BQy        
    else
        BgradQ = AQx + BQy + CQz     
...
RECURSIVE SUBROUTINE PDENCP_lk(BgradQ,Q,gradQ) 
   USE Parameters, ONLY :  nVar, nDim, epsilon1,nPar
   IMPLICIT NONE
   REAL, PARAMETER :: epsilon = 1e-14
  
   ! 11. Oct 21:40: This was a matrix BGradQ(nVar, nDim) but is a vector in spaceTimePredictorNonlinear
   REAL, INTENT(OUT) :: BgradQ(nVar,3)
   REAL, INTENT(IN)  :: gradQ(nVar, 3)
   REAL, INTENT(IN)  :: Q(nVar+nPar)
  ! Linear elasticity variables
   REAL :: lam,mu,irho, ialpha, u(3),alpha
...
! Original model
	...
	!	stop
	!end if
END SUBROUTINE PDENCP_lk

RECURSIVE SUBROUTINE PDEEigenvalues(L,Q,n)
  USE Parameters, ONLY :  nVar, nDim,nPar
  USE iso_c_binding
  IMPLICIT NONE
  REAL :: L(nVar), n(3), Q(nVar+nPar), V(nVar)
  INTENT(IN)  :: Q,n
  INTENT(OUT) :: L 
  REAL  :: lam,mu,irho
...
RECURSIVE SUBROUTINE PDEParName(MyNameOUT,ind) 
  USE Parameters, ONLY: nPar  
  IMPLICIT NONE     
  CHARACTER(LEN=10):: MyName(nPar),MyNameOUT
  INTEGER			:: ind
...
RECURSIVE SUBROUTINE getNumericalSolution(V,Q) 
  USE Parameters, ONLY: nVar,nPar  
  IMPLICIT NONE     
  REAL				:: V(nVar+nPar), Q(nVar+nPar)
...
RECURSIVE SUBROUTINE getExactSolution(V,pos, timeStamp) 
  USE Parameters, ONLY: nVar , nDim , nPar 
  IMPLICIT NONE     
  REAL				:: V(nVar+nPar), Q(nVar+nPar), pos(nDim), timeStamp
