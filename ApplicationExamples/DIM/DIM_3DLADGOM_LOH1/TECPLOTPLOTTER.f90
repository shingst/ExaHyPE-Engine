...
SUBROUTINE InitTECPLOTPLOTTER(time_in)
    USE Parameters, ONLY: nDim,nVar,nAux,nPar
	IMPLICIT NONE
	REAL, INTENT(IN) :: time_in
	ALLOCATE(NData_max(nVertex,nSubPlotElem_max))  
	ALLOCATE(DataArray_max(nRealNodes_max,nDim+nVar+nAux+nPar+1+1))
...
SUBROUTINE ElementTECPLOTPLOTTER(wh,lx0,ldx,limiter)
    USE Parameters, ONLY: nDim,nVar,nAux, nPar
	IMPLICIT NONE
	REAL, INTENT(IN) :: wh(nVar+nPar,nDOFm)
	INTEGER :: nSubNodes,J
	REAL    :: LocNode(nVar+nPar,(M+1)**nDim),xvec(3),lx0(3),ldx(3)
	REAL	:: VN(nVar+nPar),QN(nVar+nPar)
	integer :: limiter
...
!return
	...
	ALLOCATE(DataArray(nRealNodes,nDim+nVar+nAux+nPar+1+1))
	...
	DataArray=DataArray_max(1:nRealNodes,1:(nDim+nVar+nAux+nPar+1+1))
	...
	iRet = TecDat112( nRealNodes*(nDim+nVar+nAux+nPar+1+1), DataArray, visdouble )	
