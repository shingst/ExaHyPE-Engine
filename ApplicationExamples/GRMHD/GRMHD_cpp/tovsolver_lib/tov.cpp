...
void TOV::TOVSolver::TOV_C_ParamCheck()
{
  ...
  if (TOV_Fast_Interpolation == 0)
	...
	if (Perturb_Pressure[star])
      ...
      if (TOV_Combine_Method ==  "maximum")
      {
        ...
        {
          /* check metric */
          if ((my_psi4 * id.gam[0][0] < max_g_diag) &&
              (my_psi4 * id.gam[1][1] < max_g_diag) &&
              (my_psi4 * id.gam[2][2] < max_g_diag))
          {
            ...
            {
              id.gam[0][0] = max_g_diag/my_psi4;
              id.gam[1][1] = max_g_diag/my_psi4;
              id.gam[2][2] = max_g_diag/my_psi4;
          ...
          if (TOV_Use_Old_Matter_Initial_Data)
          {
            if (id.rho > max_rho)
            {
              ...
              star=0;
              max_rho          =id.rho;
              eps_point[star]  =id.eps;
              press_point[star]=id.press;
              my_velx=id.vel[0];
              my_vely=id.vel[1];
              my_velz=id.vel[2];
            }
      ...
      else if (TOV_Combine_Method == "average")
      {
         ...
         id.beta[2] = 0.0;
          if (TOV_Conformal_Flat_Three_Metric)
          {
            ...
            my_psi4 = (r_point[star_i] * r_point[star_i] /
                      ...
                      my_psi4 + pow(id.psi, 4.0) * id.gam[0][0];
            id.psi = pow(my_psi4, 0.25);
            if (!TOV_Use_Old_Initial_Data)
            {
              id.gam[0][0] = id.gam[1][1] = id.gam[2][2] = 1.0;
              id.gam[0][1] = id.gam[0][2] = id.gam[1][2] = 0.0;
            ...
            id.gam[0][0] += (r_point[star_i] * r_point[star_i] /
                         (r_to_star[star_i] * r_to_star[star_i] + 1.0e-30)) /
                        my_psi4;
          id.rho += rho_point[star_i];
          id.eps += eps_point[star_i];
          id.press += press_point[star_i];
        ...
        {
          id.gam[0][0] -= ((TOV_Num_TOVs+TOV_Use_Old_Initial_Data-1)/my_psi4);
          id.gam[1][1] = id.gam[0][0];
          id.gam[2][2] = id.gam[0][0];
        }
        ...
        if (star > -1)
        {
          id.vel[0] = TOV_Velocity_x[star];
          id.vel[1] = TOV_Velocity_y[star];
          id.vel[2] = TOV_Velocity_z[star];
        }
        ...
        id.w_lorentz = 1/sqrt(1.0-(
                                ...
                                2*id.gam[0][1] * id.vel[0] * id.vel[1]+
                                2*id.gam[0][2] * id.vel[0] * id.vel[2]+
                                2*id.gam[1][2] * id.vel[1] * id.vel[2]) * my_psi4);
      }
