...
NavierStokes::TwoBubbles::Bubble::Bubble(const double tempDifference, const double size, const double decay,
                                         const double centerX, const double centerZ) : tempDifference(tempDifference),
                                                                                         size(size), decay(decay),
                                                                                         centerX(centerX),
                                                                                         centerZ(centerZ) {}

void NavierStokes::TwoBubbles::initialValues(const double *const x, const NavierStokes &ns, Variables &vars) {
    // For details see:
    // Robert (1993),  https://doi.org/10.1175/1520-0469(1993)050<1865:BCEWAS>2.0.CO;2
    const auto posX = x[0];
    const auto posZ = (DIMENSIONS == 2) ? x[1] : x[2];

    const double backgroundPressure = ns.referencePressure; // [Pa], background pressure of atmosphere at sea level
    ...
    for (const auto& bubble: bubbles) {
        ...
        if (distanceToCenter < bubble.size) {
          ...
          potentialT += bubble.tempDifference * std::exp( - (d*d)/(bubble.decay * bubble.decay));
        }
    }

    const double g = -9.81; // [m/s^2]
    // For pressure computation assume that temperature is constant everywhere.
    const double pressure = backgroundPressure * std::exp(-(g * posZ)/(ns.gasConstant * backgroundT));

    // Conversion factor potential temperature -> temperature
    const double poTToT = std::pow((pressure/backgroundPressure), ns.gasConstant/ns.c_p);
...
void NavierStokes::TwoBubbles::source(const double *const Q, double *S) {
    Scenario::source(Q, S);
