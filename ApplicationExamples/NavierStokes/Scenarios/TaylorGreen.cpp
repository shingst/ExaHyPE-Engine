...
void NavierStokes::TaylorGreen::initialValues(const double* const x, const NavierStokes& ns, Variables& vars) {
    assert(DIMENSIONS == 2);
    // 2D-Scenario
    vars.rho() = 1.0;
    ...
    vars.j(2) = 0;
#endif
    const double pressure = -1 * (vars.rho()/4) * (std::cos(2 * x[0]) + std::cos(2 * x[1]));

    vars.E() = ns.evaluateEnergy(vars.rho(), pressure, vars.j());

}
void NavierStokes::TaylorGreen::analyticalSolution(const double* const x, const double t,
      ...
      vars.j(2) = 0;
#endif
      const auto pressure = -1 * (vars.rho()/4) * (std::cos(2 * x[0]) + std::cos(2 * x[1])) * Ft * Ft;
      vars.E() = ns.evaluateEnergy(vars.rho(), pressure, vars.j());
      ...
      gradState[idxGradQ(0, e_idx)] = e_factor * ( 1 * std::sin(2 * x[0] - 2 * x[1]) + std::sin(2 * x[0] + 2 * x[1]));
      gradState[idxGradQ(1, e_idx)] = e_factor * (-1 * std::sin(2 * x[0] - 2 * x[1]) + std::sin(2 * x[0] + 2 * x[1]));
      //gradState[idxGradQ(2, e_idx)] = 0.0;


}

NavierStokes::BoundaryType NavierStokes::TaylorGreen::getBoundaryType(int faceId) {
