...
void NavierStokesFV::MyNavierStokesSolver::adjustSolution(const double* const x,const double t,const double dt, double* Q) {
  ...
  // Number of variables    = 5 + #parameters
    if ( tarch::la::equals( t,0.0 ) ) {
        ...
        /*vars.rho() = 1.0;
        vars.j(0.0,0.0,0.0);
        vars.E() = 1./gamma + std::exp(-((x[0]-0.5)*(x[0]-0.5) + (x[1]-0.5)*(x[1]-0.5))/(0.05*0.05))*1e-3;*/
...
/*void NavierStokesFV::MyNavierStokesSolver::eigenvaluesDiffusive(const double* const Q, const int dIndex, double* lambda) {
  // Dimensions             = 2
  // Number of variables    = 5 + #parameters
  ReadOnlyVariables vars(Q);
  Variables eigs(lambda);
...
void NavierStokesFV::MyNavierStokesSolver::parabolicFlux(const double* const Q, double* gradQ, double** F) {
  // Dimensions                        = 2
