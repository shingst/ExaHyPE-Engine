...
void NavierStokes::TaylorGreen::analyticalSolution(const double* const x,
  ...
  const double pressure =
      p_0 + ((vars.rho() * Ft * Ft)/ 4) * (std::cos(2 * x[0]) + std::cos(2 * x[1]));
  ...
  // E
  gradState[idxGradQ(0, 3)] = std::pow(Ft, 2)*((1.0/4.0)*(1.0*ns.gamma - 1.0)*(std::sin(2*x[0] - 2*x[1]) + std::sin(2*x[0] + 2*x[1])) + 0.5*std::sin(2*x[0]))/(ns.gamma - 1);

  gradState[idxGradQ(1, 3)] = std::pow(Ft, 2)*((1.0/4.0)*(1.0*ns.gamma - 1.0)*(-std::sin(2*x[0] - 2*x[1]) + std::sin(2*x[0] + 2*x[1])) + 0.5*std::sin(2*x[1]))/(ns.gamma - 1);
}
