...
void SWE::MySWESolver_ADERDG_kernels::aderdg::projectOnFVLimiterSpace(const double* const luh, double* const lim) {
  
  //compact projection without ghostlayer
  // x
  double tmpX[24] __attribute__((aligned(ALIGNMENT)));
  for (int zy = 0; zy < 2; zy++) {
    // will overwrite tmpX, no need to set to 0
    #ifdef USE_IPO
    #pragma forceinline
    #endif
    gemm_4_3_2_dg2fv_x(luh+zy*8, dg2fv, tmpX+zy*12);
  }
  ...
  for (int x = 0; x < 3; x++) {
    ...
    #pragma forceinline
    #endif
    gemm_4_3_2_dg2fv_y(tmpX+x*4, dg2fv, lim+x*4);
  }
    
  //space out for ghostlayer (in place)
  for (int z = 0; z >= 0; z--) {
    for (int y = 2; y >= 0; y--) {
      std::copy_n(lim+(z*3+y)*12, 12, lim+(((z+0)*5+y+1)*5+1)*4); // no overlap since we have ghostlayers at the beginning
      std::memset(lim+(z*3+y)*12, 0, sizeof(double)*12); //delete the memory block that was moved to ensure ghostlayers are set at 0.
    }
  }
...
void SWE::MySWESolver_ADERDG_kernels::aderdg::projectOnDGSpace(const double* const lim, double* const luh) {
  ...
  for (int z = 0; z < 1; z++) {
    for (int y = 0; y < 3; y++) {
      ...
      #pragma forceinline
      #endif
      gemm_4_2_3_fv2dg_x(lim+((z+0)*5+y+1)*20+4, fv2dg, tmpX+(z*3+y)*8);
    }
  }
  ...
  for (int x = 0; x < 2; x++) {
    ...
    #pragma forceinline
    #endif
    gemm_4_2_3_fv2dg_y(tmpX+x*4, fv2dg, luh+x*4);
  }
    
}


bool SWE::MySWESolver_ADERDG_kernels::aderdg::discreteMaximumPrincipleAndMinAndMaxSearch(
...
void SWE::MySWESolver_ADERDG_kernels::aderdg::findCellLocalLimiterMinAndMax(
  ...
  for (int z = 0; z < 1; z++) { // skip the last element
    for (int y = 1; y < 4; y++) {
      for (int x = 1; x < 4; x++) {
        solver->mapDiscreteMaximumPrincipleObservables(observables,lim+(((z*5+y)*5+x)*4));
 ...
 */
void SWE::MySWESolver_ADERDG_kernels::aderdg::findCellLocalMinAndMax(
    const double* const luh,
    const exahype::solvers::ADERDGSolver* solver,
    double* const localMinPerVariables, double* const localMaxPerVariables
) {
  ...
  for (int zyx = 0; zyx < 4; zyx++) {
    solver->mapDiscreteMaximumPrincipleObservables(observables,luh+(zyx*4));
 ...
 */
void SWE::MySWESolver_ADERDG_kernels::aderdg::compareWithADERDGSolutionAtGaussLobattoNodes(
    const double* const luh,
    const exahype::solvers::ADERDGSolver* solver,
    double* min, 
    double* max
  ...
  for (int zy = 0; zy < 2; zy++) {
    ...
    #pragma forceinline
    #endif
    gemm_4_2_2_uh2lob_x(luh+zy*8, uh2lob, tmpX+zy*8);
  }
  
  // y
  double observables[0] __attribute__((aligned(ALIGNMENT)));
  double lob[8] __attribute__((aligned(ALIGNMENT)));
 // constant x slice of projected solution
  for (int x = 0; x < 2; x++) {
    ...
    #pragma forceinline
    #endif
    gemm_4_2_2_uh2lob_y_slice(tmpX+x*4, uh2lob, lob);
 
    for(int p = 0; p < 2; p++) {
      solver->mapDiscreteMaximumPrincipleObservables(observables, lob+p*4);
      for (int n = 0; n < 0; n++) {
 ...
 */
void SWE::MySWESolver_ADERDG_kernels::aderdg::compareWithADERDGSolutionAtFVSubcellCenters(
    const double* const luh,
    const exahype::solvers::ADERDGSolver* solver,
    double* min, 
    double* max
  ...
  // y
  double observables[0] __attribute__((aligned(ALIGNMENT)));
  double lim[12] __attribute__((aligned(ALIGNMENT)));
 // constant x slice of projected solution
  for (int x = 0; x < 3; x++) {
    // will overwrite lim, no need to set to 0
    #ifdef USE_IPO
    #pragma forceinline
    #endif
    gemm_4_3_2_dg2fv_y_slice(tmpX+x*4, dg2fv, lim);
    for(int p = 0; p < 3; p++) {
      solver->mapDiscreteMaximumPrincipleObservables(observables, lim+p*4);
      for (int n = 0; n < 0; n++) {
        min[n] = std::min( min[n], observables[n] );
        max[n] = std::max( max[n], observables[n] );
      }
