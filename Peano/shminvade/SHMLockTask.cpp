...
void shminvade::SHMLockTask::reenqueue() {
  if (SHMController::getInstance().getThreadType(_core) == SHMController::ThreadType::Shutdown) {
    ...
    std::cout << getSHMDebugPrefix() <<  "Controller seems to be down already, so stop lock task too (line:" << __LINE__ << ",file:" << __FILE__ << ")" << std::endl;
    #endif
  }
  else {
...
void shminvade::SHMLockTask::terminate() {
  #if SHM_INVADE_DEBUG>=8
  std::cout << getSHMDebugPrefix() <<  "Task for core " << _core << " terminates now (line:" << __LINE__ << ",file: " << __FILE__ << ")" << std::endl;
  #endif
...
tbb::task* shminvade::SHMLockTask::execute() {
  ...
  switch (state) {
    case SHMController::ThreadType::Master:
      ...
      std::cout << getSHMDebugPrefix() <<  "Core " << _core << " is the master. Retreat immediately as lock task found on core " << currentCore << " (line:" << __LINE__ << ",file: " << __FILE__ << ")" << std::endl;
      #endif
      terminate();
      return nullptr;
    case SHMController::ThreadType::Owned:
      ...
      #if SHM_INVADE_DEBUG>=8
      std::cout << getSHMDebugPrefix() <<  "Core " << _core << " is owned by process. Lock task found on core " << currentCore << ". Terminate (line:" << __LINE__ << ",file: " << __FILE__ << ")" << std::endl;
      #endif
      terminate();
      return nullptr;
      case SHMController::ThreadType::NotOwned:
        if ( currentCore!=_core ) {
          #if SHM_INVADE_DEBUG>=8
    	  std::cout << getSHMDebugPrefix() <<  "Lock task for core " << _core <<
          ...
          std::cout << getSHMDebugPrefix() <<  "Thread on core " << _core << " should not be used. Make lock task sleep for " << _sleepTime << "s before we reenqueue (line:" << __LINE__ << ",file: " << __FILE__ << ")" << std::endl;
          #endif
          sleep(_sleepTime);
          reenqueue();
          return nullptr;
      ...
      case SHMController::ThreadType::Shutdown:
        ...
        std::cout << getSHMDebugPrefix() <<  "Core " << _core << " is marked to shut down. Lock task found on core " << currentCore << ". Terminate (line:" << __LINE__ << ",file: " << __FILE__ << ")" << std::endl;
        #endif
